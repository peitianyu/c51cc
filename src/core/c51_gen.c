...
static unsigned reg_bit(int r)
{
    if (r < 0 || r > 7) return 0;
    return 1u << r;
}

static unsigned reg_mask_from_arg(const char *arg)
{
...
static void reg_use_def(const AsmInstr *ins, unsigned *use, unsigned *def)
{
    if (use) *use = 0;
    if (def) *def = 0;
...
static void shrink_call_saves(Section *sec)
{
    if (!sec || !sec->asminstrs) return;
...
static const char *invert_jcc(const char *op)
{
    if (!op) return NULL;
