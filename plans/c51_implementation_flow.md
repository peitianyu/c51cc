...
```bash
# 编译单个测试
echo "int x = 10; void f() {}" | ./c51cc -o test.asm
...
```c
#ifdef C51_DEBUG
#define DEBUG(fmt, ...) fprintf(stderr, "[C51] " fmt "\n", ##__VA_ARGS__)
#else
#define DEBUG(...)
#endif
```
...
| 风险 | 应对策略 |
|------|----------|
| 寄存器分配复杂度高 | 先实现最简单版本（无溢出），再迭代优化 |
| 8051指令集限制 | 优先实现核心指令，复杂操作用指令序列模拟 |
| 内存模型多样 | 先支持SMALL模式，再扩展LARGE/COMPACT |
| 中断函数特殊处理 | 单独阶段处理，使用 `using` 关键字 |
