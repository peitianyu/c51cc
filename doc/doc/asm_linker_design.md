...
4) 【已完成】有符号比较与位移降级：补齐 `LT/LE/GT/GE` 的符号语义与算术右移。
  - 测试：符号/无符号对比样例。
5) 【已完成】地址空间访问：`MOV/MOVX/MOVC` 按类型属性选择。
  - 测试：xdata/code 指针访问样例。
6) 【已完成】拓展sfr等: 使用register bool/char/int等映射到特殊寄存器与位操作。
  - 测试：sfr/sbit 访问样例（test_all.c）。
7) 【已完成】窥孔优化扩展：短跳转选择、冗余 load/store 清理。
  - 说明：无条件跳转超距自动升级为 ljmp；冗余寄存器往返 mov 清理。
  - 测试：控制流与短跳转样例（test_all.c）。
8) 【已完成】伪指令扩展：完善剩余伪指令与汇编器诊断。
  - 说明：新增 .space/.interrupt/.using/.reentrant，参数校验。
  - 测试：汇编器解析错误/边界输入。
