...
```
    ...
    不再使用interrupt, using关键字, 直接使用特定函数表示, void interupt_func(int inerrupt_id, int bank_id)
    因此语法将不直接支持keil等
```
...
- static 分为`file.static`与`func.static` 这部分将直接在json的globals中体现
...
✔ 创建预处理器模块 (pp.c) @done(26-01-28 09:26)
    ...
    ☐ 宏展开递归保护 - 防止无限递归
    ☐ 函数式宏参数展开 - 当前只解析不展开参数
✔ 处理由于预处理导致的行号/文件名变化 @done(26-01-29 17:36)
```