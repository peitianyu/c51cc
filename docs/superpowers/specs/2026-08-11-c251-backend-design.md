...
- **指令集权威参考**：本仓库 `docs/stc32g_指令功能排序.md`（510 行）——STC32G 指令按功能分类（算术/逻辑/布尔/数据传输/程序跳转 5 类），每条带**编码 + 字节数 + 时钟数**，标注 🔴BINARY/🔵SOURCE 模式专用；作为 Source Mode 指令选择与编码表的首选参考，与 sim251 `decode_impl.inc` + `functional.py` 交叉验证
