...
int c251_write_hex(FILE *fp, const ObjFile *obj) {
    if (!fp || !obj) return -1;
    /* M1: CODE section 从地址 0 连续输出；EDATA 数据区也输出（sim251 支持 16 位地址 hex） */
    unsigned addr = 0;
    for (Iter sit = list_iter(obj->sections); !iter_end(sit);) {
        Section *sec = iter_next(&sit);
        if (!sec) continue;
        if (sec->kind == SEC_CODE && sec->bytes_len > 0) {
            int off = 0;
    ...
    hex_emit_line(fp, 0, NULL, 0); /* EOF 记录 */
    return 0;
}
```
...
```python
# 验证: MOV WR6,#0x1234 (7E 64 12 34) + RET (22) → sim251 执行后 WR6=0x1234
import subprocess, tempfile, os
hexdata = ":040000007E64123422\n:00000001FF\n"
...
/* 值宽度: 1=字节, 2=字, 4=双字 (M1 只支持 1/2) */
static int value_size(ISelContext* isel, ValueName val) {
    char key[32]; snprintf(key, sizeof(key), "%d", val);
    Ctype *t = dict_get(isel->ctx->value_type, key);
    if (!t) return 2;
    return t->size > 1 ? 2 : 1;
}
...
void isel_block(ISelContext* isel, Block* block) {
    ...
    for (Iter it = list_iter(block->instrs); !iter_end(it);) {
        ...
        /* 简单前瞻: next = 下一条（M1 简化） */
        Instr *next = NULL;
        if (!iter_end(it)) next = *(Instr**)list_peek(it);
        isel_instr(isel, ins, next);
    }
}
